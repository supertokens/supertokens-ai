...
- Before showing the user the answer for a question, ask chatgpt if it thinks that the suggested answer actually answers the question without the context, and if it doesn't then try again, but with other context that are closer to the question, but far away from the previous answer.